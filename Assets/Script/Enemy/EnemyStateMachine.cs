...
public class EnemyStateMachine : MonoBehaviour
{
    ...
    bool isAllowedToAttack; // = !isCastingSkill
    ...
    //debugging method for built game & temporary bug solvers 
    ...
    private float battleSceneAccumulator=0f;  //indiciate how long it stays in the battle scene from start 
    private float attackInitialTime =0.3f;
//initialization
...
// The Main Finite State Reacting function
    void EnemyReactWithState(){
        if(sceneHandler.IsInMainScene()){  //MainScene StateMachine
            ...
            battleSceneAccumulator+=Time.deltaTime;
...
// State Checking helper 
    ...
    bool CanAttack(){   //suppose skill have been chosen
        if(isAllowedToAttack && IsInsideAttackRegion() && enemyVision.CanSee(Player.gameObject)&&battleSceneAccumulator>attackInitialTime)        //stop and attack
            return true; 
        else
            return false;
    }
